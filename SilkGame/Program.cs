...
public class Program {
	...
	private static unsafe void OnLoad() {
		...
		_gl.ClearColor(Color.BlanchedAlmond);
		...
		if (vStatus != (int)GLEnum.True) {
			throw new("Vertex shader failed to compile: " + _gl.GetShaderInfoLog(vertexShader));
		}
		...
		if (fStatus != (int)GLEnum.True) {
			throw new("Fragment shader failed to compile: " + _gl.GetShaderInfoLog(fragmentShader));
		}
		...
		if (lStatus != (int)GLEnum.True) {
			throw new("Program failed to link: " + _gl.GetProgramInfoLog(_program));
		}
		...
		fixed ( byte* ptr = result.Data ) {
			...
			_gl.TexImage2D(
				TextureTarget.Texture2D, 0, InternalFormat.Rgba, (uint)result.Width,
